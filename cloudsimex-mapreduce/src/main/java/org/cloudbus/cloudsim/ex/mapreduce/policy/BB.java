	...
	public Boolean runAlgorithm(Cloud cloud, Request request, int numCostTrees, boolean enablePerfTree,
		...
		Collections.sort(nVMs, new Comparator<VmInstance>() {
			...
			public int compare(VmInstance VmInstance1, VmInstance VmInstance2) {
				double vmInstance1Cost = VmInstance1.transferringCost
						+ VmInstance1.vmCostPerHour / VmInstance1.getMips();
				double vmInstance2Cost = VmInstance2.transferringCost
						+ VmInstance2.vmCostPerHour / VmInstance2.getMips();
		...
		if (backtrackingType == BacktrackingType.Decision) {
			BBDecisionAlgorithm backTrackingDecisionAlgorithm = new BBDecisionAlgorithm(request, cloud, nVMs, rTasks,
					loggingFrequent, forceToAceeptAnySolutionTimeMillis, forceToExitTimeMillis);
		...
		double[] executionTimeAndCost = predictionEngine
				.predictExecutionTimeAndCostFromScheduleingPlan(selectedSchedulingPlan, nVMs);
		CustomLog.printLine("Selected Solution: " + Arrays.toString(solutionVector) + " : "
				+ Arrays.toString(executionTimeAndCost));

		// 1- Provisioning
		ArrayList<ArrayList<VmInstance>> provisioningPlans = predictionEngine
				.getProvisioningPlan(selectedSchedulingPlan, nVMs, request.job);
