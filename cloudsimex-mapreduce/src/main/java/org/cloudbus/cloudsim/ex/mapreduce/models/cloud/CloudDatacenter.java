...
public class CloudDatacenter extends Datacenter {
	...
	public CloudDatacenter(String name, int hosts, int memory_perhost, int cores_perhost, int mips_precore_perhost,
		...
		super(name, createChars(hosts, cores_perhost, mips_precore_perhost, memory_perhost, 1000000),
				new VmAllocationPolicySimple(
						getHostList(hosts, cores_perhost, mips_precore_perhost, memory_perhost, 1000000)),
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (cl instanceof MapTask) {
				...
				Log.printLine(CloudSim.clock() + ": " + getName() + ": Estimated Total Execution Time for MAP task ID: "
						+ cl.getCloudletId() + " is: " + mapTask.dataTransferTimeFromTheDataSource() + "(D-in) + "
