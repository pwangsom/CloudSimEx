...
public abstract class Policy {
	...
	private final static int MAX_VM = 8;
...
/*		if (cloudDeploymentModel == CloudDeploymentModel.Public) {
		...
		if (cloudDeploymentModel == CloudDeploymentModel.Public
				|| cloudDeploymentModel == CloudDeploymentModel.Hybrid) {
			for (PublicCloudDatacenter publicCloudDatacenter : cloud.publicCloudDatacenters) {
				for (VmType vmType : publicCloudDatacenter.vmTypes)
					for (int i = 0; i < numberOfDuplicates; i++)
						nVMs.add(new VmInstance(vmType, request));
		...
		if (cloudDeploymentModel == CloudDeploymentModel.Private
				|| cloudDeploymentModel == CloudDeploymentModel.Hybrid) {
	...
	public static List<VmInstance> getAllVmInstances(Cloud cloud, Request request, int numberOfDuplicates) {
		List<VmInstance> nVMs = new ArrayList<VmInstance>();
