...
public class MapReduceEngine extends DatacenterBroker {
	...
	public void processEvent(SimEvent ev) {
		...
		case CloudSimTags.VM_CREATE:
			printLogOfProcessEvent("CloudSimTags.VM_CREATE");
	...
	protected void vmProvisioning_taskScheduling(Request request) {
		...
		Log.printLine(" =========== SEARCHING START USING ALGORITHM:" + request.policy + "-"
				+ request.getCloudDeploymentModel() + " FOR REQUEST: " + request.id + requestString + " ===========");
		...
		if (policy == null || !policy.runAlgorithm(cloud, request)){
			Log.printLine(
					" =========== ERROR: THE ALGORITHM COULD NOT FIND VMs FOR REQUEST: " + request.id + " ===========");
		} else {
	...
	private void startReducePhase(Request request) {
		...
		if (request.reduceOnlyVmProvisionList.size() != 0) {
			...
			Log.printLine(
					getName() + " PROVISIONING NOW THE FOLLOWING VMs (Reduce Only) [Request ID: " + request.id + "]:-");
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		Log.printLine(
				CloudSim.clock() + ": " + getName() + ": Cloudlet " + cloudlet.getCloudletId() + " finished executing");
	...
	public void logExecutionSummary() {
		...
		for (Request request : requests.requests) {
			CustomLog.printResultsWithoutHeader(VmInstance.class, ",",
					new String[] { "ExperimentNumber", "WorkloadNumber", "RequestId", "J", "UserClass", "Policy",
							"CloudDeploymentModel", "Id", "Name", "ExecutionTime", "ExecutionCost", "TasksIdAsString" },
					request.mapAndReduceVmProvisionList);
			CustomLog.printResultsWithoutHeader(VmInstance.class, ",",
					new String[] { "ExperimentNumber", "WorkloadNumber", "RequestId", "J", "UserClass", "Policy",
							"CloudDeploymentModel", "Id", "Name", "ExecutionTime", "ExecutionCost", "TasksIdAsString" },
					request.reduceOnlyVmProvisionList);
		}
		// tasks.csv logging
		CustomLog.redirectToFile("output/logs/tasks.csv", true);
		CustomLog.printResultsWithoutHeader(Task.class, ",",
				new String[] { "ExperimentNumber", "WorkloadNumber", "RequestId", "J", "UserClass", "Policy",
						"CloudDeploymentModel", "CloudletId", "TaskType", "CloudletLength", "CloudletStatusString",
		...
		CustomLog.printResultsWithoutHeader(Request.class, ",",
				new String[] { "ExperimentNumber", "WorkloadNumber", "Id", "J", "UserClass", "Policy",
